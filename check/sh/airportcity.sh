...
do
	...
	do
		...
		while [ $page -gt 0 ]
		do
			htm=$(browse "$url/www.frankfurt-airport.com/?type=arrival&time=$(rawurlencode $YYYYmmddTHHMMSSZ)&items=3&page=$page")
			next=$(awk 'BEGIN {
					page = 0;
				}
...
do
	...
	check "$day-0600-fia" browse "$url/www.frankfurt-airport.com/?time=$(rawurlencode $YYYYmmddTHHMMSSZ)\&fia=SA260$YYYYmmdd" \
		"| sed -r 's/[0-9]{2}\\.[0-9]{2}\\.[0-9]{4}/00.00.0000/g'"

	check "$day-0600-fid" browse "$url/www.frankfurt-airport.com/?time=$(rawurlencode $YYYYmmddTHHMMSSZ)\&fid=SA261$YYYYmmdd" \
		"| sed -r 's/[0-9]{2}\\.[0-9]{2}\\.[0-9]{4}/00.00.0000/g'"
done
...
check "1-2300-fia" browse "$url/www.frankfurt-airport.com/?time=$(rawurlencode $YYYYmmddTHHMMSSZ)\&fia=SA260$YYYYmmdd" \
	"| sed -r 's/[0-9]{2}\\.[0-9]{2}\\.[0-9]{4}/00.00.0000/g'"

check "1-2300-fid" browse "$url/www.frankfurt-airport.com/?time=$(rawurlencode $YYYYmmddTHHMMSSZ)\&fid=SA261$YYYYmmdd" \
	"| sed -r 's/[0-9]{2}\\.[0-9]{2}\\.[0-9]{4}/00.00.0000/g'"
